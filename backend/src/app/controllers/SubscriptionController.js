...
class SubscriptionController {
    ...
    try {
      ...
      if (meetup.past) {
        return res.status(400).json({
          error: "You're can't subscribe to past Meetups.",
        });
      }
      ...
      if (hasMeetupOnTheSameDay) {
        return res.status(400).json({
          error: "You're already are subscribed to a Meetup at the same date",
        });
      }
  ...
  async delete(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ success: 'Your subscription has been canceled' })
        .send();
    } catch (err) {
      return res.status(400).json(err.message);
    }
